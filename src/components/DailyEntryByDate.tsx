 ...
 */
function DailyEntryByDate({ day }: DailyEntryProps): JSX.Element {
  const { data, error } = useSWR(
    day
      ? `query DailyEntryByDate($day: date) {
        daily_entries(where: {day: {_eq: "${day}"}}) {
        id
        created: created_at
        summary: day_summary
...
export default DailyEntryByDate;
